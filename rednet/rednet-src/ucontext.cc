...
ucontext::~ucontext()
{
    ...
    uglobal::instance()->contextDec();
}

uint32_t ucontext::init(const char *name, const char *parm)
{
    umodule *dl = UMODS_CALL->query(name);
    if (!dl)
        return 0;
    ucomponent *hcomp = dl->create_cb__();
    if (!hcomp)
        return 0;
    id = UCTXS_CALL->registerId(this);
    if (id == 0)
        return 0;
    md__ = dl;
    cp__ = hcomp;
    ...
    if (r == 0)
    {
        UCTXS_CALL->registerName(id, name);
        uglobal::instance()->contextInc();
        inited__ = true;
        EQ_CALL->push(eq);
        ucontext::error(id, "LANUCH %s %s", name, parm ? parm : "");
        return id;
    }
    else
    {
        ucontext::error(id, "FAILED started %s", name);
        UCTXS_CALL->unRegister(id);
        return 0;
    }
}
...
void ucontext::execute(uevent *e)
{
    ...
    else
    {
        ...
        if (profile__)
        {
            cpu_start__ = timestamp::getThreadTime();
            result = cp__->callback__(cp__->ud__, type, e->source, e->session, e->data, sz);
            uint64_t cost_ts = timestamp::getThreadTime() - cpu_start__;
            cpu_cost__ += cost_ts;
        }
        else
            result = cp__->callback__(cp__->ud__, type, e->source, e->session, e->data, sz);
...
{
    static uint32_t logger = UCTXS_CALL->convertId("logger");
    if (logger == 0)
        return;
    ...
    ctx_ptr logger_ptr = UCTXS_CALL->grab(logger);
...
ucontexts::ucontexts() : ctxsSer__(1),
                         ctxsCap__(4)
{
...
void ucontexts::init(int serverGroupID)
{
    serverGroupID__ = (uint32_t)(serverGroupID & 0xff) << ID_REMOTE_SHIFT;
}

uint32_t ucontexts::registerId(ucontext *ctx)
{
    ctxsLck__.wlock();
    for (;;)
    {
...
const char *ucontexts::registerName(uint32_t id, const char *name)
{
...
bool ucontexts::unRegister(uint32_t id)
{
    bool bret = false;
    wlocking wk(&ctxsLck__);
...
ctx_ptr ucontexts::grab(uint32_t id)
{
...
uint32_t ucontexts::convertId(const char *name)
{
    rlocking rk(&ctxsLck__);
    if (ctxsName__.empty())
        return 0;
...
const char *ucontexts::convertName(uint32_t id)
{
    rlocking rk(&ctxsLck__);
    if (ctxsName__.empty())
        return NULL;
...
void ucontexts::local_erase_name(uint32_t id)
{
    if (ctxsName__.empty())
        return;
    umaps::iterator it = ctxsName__.begin();
