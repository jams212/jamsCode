...
uint32_t ucall::addrTo(const char *addr)
{
    ...
    {
        dest = UCTXS_CALL->convertId(addr + 1);
        if (dest == 0)
        {
...
{
    ctx_ptr ptr = UCTXS_CALL->grab(id);
    if (ptr == nullptr)
        return false;
...
const char *ucall::luaTimeOut(ucontext *ctx, const char *param)
{
    ...
    CLOCK_CALL->timeOut(ctx->id, ti, session);
...
const char *ucall::luaName(ucontext *ctx, const char *param)
{
    ...
    if (name[0] == '.')
    {
        return UCTXS_CALL->registerName(id, name + 1);
    }
    else
    {
...
const char *ucall::luaQuery(ucontext *ctx, const char *param)
{
    if (param[0] == '.')
    {
        uint32_t id = UCTXS_CALL->convertId(param + 1);
        if (id)
        {
...
const char *ucall::luaSignal(ucontext *ctx, const char *param)
{
    ...
    if (handle == 0)
        return NULL;
    ctx_ptr obj_ent = UCTXS_CALL->grab(handle);
    if (obj_ent == nullptr)
        return NULL;
    param = strchr(param, ' ');
    int sig = 0;
