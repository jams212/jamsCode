...
umodules *__g_mods = NULL;

umodules::umodules() : modp__(NULL)
{
}
...
void umodules::init(const char *cpath)
{
    if (!cpath)
    {
...
umodule *umodules::query(const char *name)
{
...
umodule *umodules::local_query(const char *name)
{
    rlocking rl(&modk__);
    if (mods__.empty())
        return NULL;
...
umodule *umodules::local_register(const char *name)
{
    wlocking wl(&modk__);
    if (!mods__.empty())
    {
...
void *umodules::local_opendl(const char *name)
{
    const char *l;
